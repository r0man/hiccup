...
;; (let [attrs2436 (str "bar" "baz")]
;;   (if (map? attrs2436)
;;     (str
;;      "<div"
;;      (render-attr-map (merge {:id nil, :class "foo"} attrs2436))
;;      ">"
;;      "</div>")
;;     (str "<div class=\"foo\">" (render-html attrs2436) "</div>")
;;     )
;;   (render-attr-map (merge {:id nil, :class "foo"} attrs2436)))

(defmethod render-html LazySeq [coll]
  (apply str (map render-html coll)))

(defmethod render-html :default [x]
  (.log js/console (str "RENDER-HTML: " x))
  (as-str x))
