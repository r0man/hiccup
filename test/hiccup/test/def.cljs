...
(deftest test-defelem
  (testing "one overload function"
    ...
    ;; (is (thrown? js/Error (one-form-two-args)))
    ;; (is (thrown? js/Error (one-form-two-args {})))
    ;; (is (thrown? js/Error (one-form-two-args 1)))
    (is (= [1 0 3] (one-form-two-args 0 1)))
    (is (= [1 {:foo :bar} 0 3] (one-form-two-args {:foo :bar} 0 1)))
    ;; (is (thrown? js/Error (one-form-two-args 1 2 3)))
    ;; (is (thrown? js/Error (one-form-two-args 1 2 3 4)))
    )
  (testing "three overloads function"
    ...
    (is (thrown? js/Error (three-forms 1 2 3)))
    (is (thrown? js/Error (three-forms 1 2 3 4))))
  ...
  (testing "preserve meta info"
    (defelem three-forms-extra
      ...
      ([a] {:pre [(> a 1)]} [1])
      ([a b] {:pre [(> a 1)]} [1 2]))
    (is (thrown? js/Error (three-forms-extra)))
    (is (thrown? js/Error (three-forms-extra 0)))
    (is (thrown? js/Error (three-forms-extra 0 0)))
    ;; TODO: ClojureScript?
